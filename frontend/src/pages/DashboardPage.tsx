...
export default function DashboardPage() {
  ...
  const loadStatus = useCallback(async () => {
    try {
      ...
      setSdkConnected(data.sdkConnected ?? false);
  ...
  const handleMasterConnected  = (info: AccountInfo) => { setMaster(info); setSdkConnected(true); setShowConnect(false); };
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            {(!master || !sdkConnected) && (
              <button onClick={() => setShowConnect(true)}
                ...
                {master && !sdkConnected ? 'Reconectar Bullex' : 'Conectar conta Bullex'}
              </button>
            )}
          ...
          {(!master || !sdkConnected) && (
            <div className="flex items-center gap-3 px-6 py-3 bg-amber-50 dark:bg-amber-500/10 border-b border-amber-200 dark:border-amber-500/20">
              ...
              <p className="text-sm text-amber-700 dark:text-amber-300 flex-1">
                {master && !sdkConnected
                  ? <><strong>Sessão com a corretora expirada.</strong> O servidor reiniciou ou a sessão expirou. Reconecte para continuar o copy trading.</>
              ...
              <button onClick={() => setShowConnect(true)}
                ...
                {master && !sdkConnected ? 'Reconectar →' : 'Conectar agora →'}
              </button>
            </div>
          )}
        ...
        <div className="col-span-12 lg:col-span-8 xl:col-span-9 min-w-0 overflow-hidden">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-[var(--text-3)] text-xs">Atualizado via WebSocket em tempo real</p>
            </div>
    ...
    <div className="max-w-2xl space-y-5">
      ...
      <div className="card">
        ...
        <div className="p-5 grid grid-cols-2 gap-3">
          {[
            { k: 'Plataforma', v: 'Bull-ex' },
            { k: 'API',        v: 'api.trade.bull-ex.com' },
            { k: 'SDK',        v: '@quadcode-tech/client-sdk-js' },
            { k: 'Versão',     v: '2.0.0-scalable' },
          ].map(({ k, v }) => (
