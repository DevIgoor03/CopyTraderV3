...
export function MobileDockNav({ items, activeId, onSelect, showLiveOnFirst, onLogout }: Props) {
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 z-50 flex h-[calc(3.75rem+env(safe-area-inset-bottom,0px))] items-stretch border-t border-[oklch(0.88_0.02_145)]/95 bg-gradient-to-t from-[oklch(0.99_0.008_150)] to-white pb-[env(safe-area-inset-bottom,0px)] shadow-[0_-4px_24px_-8px_rgba(0,0,0,0.08)] dark:border-white/[0.08] dark:from-[oklch(0.065_0.012_155)] dark:to-[oklch(0.078_0.014_155)] dark:shadow-[0_-8px_32px_-12px_rgba(0,0,0,0.5)] md:hidden"
          ...
          return (
            <button
              ...
              className={`relative flex min-w-0 flex-1 flex-col items-center justify-center gap-0.5 rounded-xl py-1 transition-colors ${
                active
                  ? 'text-[oklch(0.22_0.06_155)] dark:text-[oklch(0.94_0.006_155)]'
                  : 'text-[oklch(0.48_0.02_150)] dark:text-[oklch(0.52_0.018_152)]'
              }`}
            >
              {showPulse && (
