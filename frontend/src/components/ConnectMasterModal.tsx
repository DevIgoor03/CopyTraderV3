    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-2xl w-full max-w-md animate-scale-in">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-[var(--text-2)] mb-1.5">Email Bull-ex</label>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
              placeholder="master@email.com" required className="field w-full" />
          </div>

          <div>
            <label className="block text-xs font-medium text-[var(--text-2)] mb-1.5">Senha Bull-ex</label>
