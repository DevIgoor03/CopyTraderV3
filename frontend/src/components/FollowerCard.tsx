    ...
    <>
      <div className={`card transition-all duration-200 ${!cs.isActive ? 'opacity-60' : ''} ${removing ? 'scale-95 opacity-0' : ''}`}>
        ...
        <div className="px-4 pb-4 pt-3 flex flex-wrap gap-1.5">
          <Chip label={`${modeLabel[cs.mode]}: ${cs.mode === 'fixed' ? `$${cs.amount}` : `${cs.amount}%`}`} />
          {cs.stopWin  != null && <Chip label={`↑ SW $${cs.stopWin}`}  cls="badge-green" />}
          {cs.stopLoss != null && <Chip label={`↓ SL $${cs.stopLoss}`} cls="badge-red"   />}
