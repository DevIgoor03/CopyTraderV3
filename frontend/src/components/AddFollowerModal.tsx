...
export default function AddFollowerModal({ onClose, onAdded }: Props) {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const data = await accountsApi.addFollower(email, password, cfg);
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-c">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-c3 text-xs">Credenciais da conta Bull-ex</p>
            </div>
          </div>
          ...
          <div className="space-y-3.5">
                  ...
                  <button key={t} type="button" onClick={() => set('accountType', t)}
                    className={`py-2.5 rounded-xl text-sm font-semibold border transition-all ${
                      cfg.accountType === t
                        ...
                        : 'bg-surface2 border-c text-c3 hover:text-c2'
                    }`}>
                    {t === 'demo' ? '🎯 Demo' : '💰 Real'}
                  </button>
                ))}
            ...
            <div>
              <label className="label">
                {cfg.mode === 'fixed' ? 'Valor (USD)' : cfg.mode === 'multiplier' ? 'Multiplicador (ex: 2 = 2x)' : '% do saldo'}
              </label>
              <input type="number" value={cfg.amount}
                     onChange={(e) => set('amount', parseFloat(e.target.value) || 0)}
                     min={0} step="any" className="field" disabled={loading} />
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="label" style={{ color: '#65a30d' }}>↑ Stop Win ($)</label>
              ...
              <div>
                <label className="label" style={{ color: '#ef4444' }}>↓ Stop Loss ($)</label>
                <input type="number" placeholder="Sem limite" min={0} step="any"
