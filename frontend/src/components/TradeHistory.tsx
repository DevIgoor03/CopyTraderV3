import { Clock, Trash2, ChevronUp, ChevronDown, TrendingUp, TrendingDown } from 'lucide-react';
...
export default function TradeHistory({ trades, onClear, compact = false, showClear = true }: Props) {
  const handleClear = async () => {
    try { await accountsApi.clearHistory(); onClear(); toast('Histórico limpo', { icon: '🗑️' }); }
  ...
  return (
    <div className="card overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-c">
        ...
        {!compact && showClear && trades.length > 0 && (
          <button onClick={handleClear} className="btn-ghost text-xs hover:text-red-500 px-2 py-1.5 flex-shrink-0">
  ...
  return (
    <tr className="hover:bg-surface2 transition-colors animate-fade-in">
      {!compact && (
        <td className="px-4 py-3 whitespace-nowrap">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-c2 font-medium">{t.followerName ?? 'Seguidor'}</span>
          </div>
        </td>
      )}
      <td className="px-4 py-3 whitespace-nowrap">
      ...
      <td className="px-4 py-3 text-right whitespace-nowrap">
        <span className="num text-sm text-c1 font-semibold">${t.amount.toFixed(2)}</span>
      </td>
      <td className="px-4 py-3 whitespace-nowrap">
        {isPend ? (
          ...
          <div className="inline-flex items-center gap-1.5">
            ...
            <span className="num text-xs text-brand-600 dark:text-brand-400 font-semibold">+${t.profit.toFixed(2)}</span>
          </div>
        ) : (
          <div className="inline-flex items-center gap-1.5">
            ...
            <span className="num text-xs text-red-500 dark:text-red-400 font-semibold">-${Math.abs(t.profit).toFixed(2)}</span>
          </div>
        )}
      </td>
      <td className="px-4 py-3 text-right whitespace-nowrap">
