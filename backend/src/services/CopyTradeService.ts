...
import { PLAN_CATALOG, planService } from './PlanService.js';
...
export class CopyTradeService {
  ...
  async start(masterId: string): Promise<void> {
    ...
    if (!facade) throw new Error('Posições do master não inicializadas. Conecte a conta Bull-ex primeiro.');
  ...
  private async _replicatePosition(masterId: string, masterPos: Position): Promise<void> {
    ...
    logger.info(
      ...
      '📋 Replicating position to followers'
    );
    ...
    // Plano: ondas paralelas + delay entre ondas (START mais lento, ELITE quase instantâneo).
    let spec = PLAN_CATALOG.START;
    try {
      spec = (await planService.getPlanForMasterId(masterId)).spec;
    } catch {
      /* master sem linha em DB raro */
    }

    logger.debug(
      { masterId, plan: spec.id, waveDelay: spec.copyWaveDelayMs, perWave: spec.followersPerWave, initial: spec.initialCopyDelayMs },
      'Copy plan execution profile'
    );

    if (spec.initialCopyDelayMs > 0) {
      await new Promise((r) => setTimeout(r, spec.initialCopyDelayMs));
    }

    const wave = Math.max(1, spec.followersPerWave);
    for (let i = 0; i < followers.length; i += wave) {
      const chunk = followers.slice(i, i + wave);
      ...
      if (i + wave < followers.length && spec.copyWaveDelayMs > 0) {
        await new Promise((r) => setTimeout(r, spec.copyWaveDelayMs));
      }
    }
  }
  ...
  private async _openBlitz(
    ...
    const expSize = times.includes(masterExpSecs) ? masterExpSecs : times[0] ?? 30;

    const dir = (masterPos.direction ?? '').toLowerCase() === 'put'
      ? BlitzOptionsDirection.Put : BlitzOptionsDirection.Call;

    logger.debug({ followerId, activeId: active.id, expSize, dir, amount }, 'Opening blitz position...');
  ...
  private async _openTurbo(
    ...
    logger.debug({ followerId, activeId: active.id, expiredAt: instrument.expiredAt, dir, amount }, 'Opening turbo position...');
  ...
  private async _resolveFollowerTrade(
    masterId: string,
    trade: { id: string; followerId: string; amount: number; openedAt: Date },
    closeReason: string,
  ): Promise<void> {
    ...
    logger.info(
      { tradeId: trade.id, followerId: trade.followerId, status, profit, closeReason },
      '✅ Follower trade resolved via own SDK'
    );

    const follower = accountService.getFollower(trade.followerId);
    if (follower) {
      ...
      if (shouldCount) {
        const s     = (accountService.getFollower(trade.followerId) ?? follower).sessionStats;
        ...
        await accountService.updateFollowerStats(trade.followerId, stats);
        await accountService.refreshFollowerBalance(trade.followerId);
      }

      const fresh = accountService.getFollower(trade.followerId);
      if (fresh) await this._checkStopConditions(masterId, trade.followerId, fresh);
    }
  ...
  private async _resolveTrades(masterId: string, masterPosId: number, closeReason: string): Promise<void> {
    const openTrades = await prisma.trade.findMany({
      where: { masterId, masterPositionId: String(masterPosId), status: 'OPEN' },
    });

    if (openTrades.length === 0) return;

    // Filter out followers whose trades will be (or were) resolved by their own subscription.
    const fallbackTrades = openTrades.filter((t) => !this.followerPositionSubscribed.has(t.followerId));
    ...
    for (const trade of fallbackTrades) {
      ...
      if (follower) {
        ...
        if (shouldCountInSession) {
          const refreshedFollower = accountService.getFollower(trade.followerId) ?? follower;
          ...
          logger.debug(
            { tradeId: trade.id, followerId: trade.followerId, openedAt: trade.openedAt, startedAt: follower.sessionStats.startedAt },
