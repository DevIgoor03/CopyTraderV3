...
export class AccountService {
  ...
  async disconnectMaster(masterId: string): Promise<void> {
    ...
    await prisma.masterAccount.update({ where: { id: masterId }, data: { isConnected: false, copyRunning: false } });
  ...
  async registerFollowerFromPortal(masterId: string, bullexEmail: string, bullexPassword: string): Promise<FollowerAccount> {
    ...
    const settings: CopySettings = existing
      ...
      : { mode: 'fixed', amount: 5, accountType: 'demo', isActive: false, stopWin: null, stopLoss: null };
  ...
  async restoreFromDb(): Promise<void> {
    ...
    logger.info({ count: masters.length }, 'Restoring master accounts from DB...');
    ...
    for (const m of masters) {
      await prisma.masterAccount.update({
        where: { id: m.id },
        data:  { isConnected: false, copyRunning: false },
      }).catch(() => {});
  ...
  async updateFollowerSettings(followerId: string, settings: Partial<CopySettings>): Promise<FollowerAccount | null> {
    ...
    if (!conn) {
      ...
      if (!f) return null;
      await prisma.followerAccount.update({ where: { id: followerId }, data: this._settingsToDb(settings) });
      return null;
    }
    conn.account.copySettings = { ...conn.account.copySettings, ...settings };
    await prisma.followerAccount.update({ where: { id: followerId }, data: this._settingsToDb(settings) });
  ...
  private _settingsToDb(s: Partial<CopySettings>): Record<string, any> {
    ...
    if (s.accountType !== undefined) d.accountType = s.accountType.toUpperCase();
